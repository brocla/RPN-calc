...
class DisplayFormatterTest {
    ...
    @Test fun fix2_largeOverflow() {
        ...
        assertTrue(result.contains("e") || result.contains("E"), "Expected SCI fallback, got: $result")
    }

    @Test fun fix2_smallNoOverflow() = assertEquals("0.001", fmt.format(state(0.001, DisplayMode.Fix(2))))
    ...
    @Test fun sci2_normal() = assertEquals("1.23e+04", fmt.format(state(12345.0, DisplayMode.Sci(2))))
    @Test fun sci0_normal() = assertEquals("1e+04", fmt.format(state(12345.0, DisplayMode.Sci(0))))
    @Test fun sci2_negative() = assertEquals("-1.23e+04", fmt.format(state(-12345.0, DisplayMode.Sci(2))))
    @Test fun sci2_small() = assertEquals("1.23e-03", fmt.format(state(0.00123, DisplayMode.Sci(2))))
    @Test fun sci2_exactlyOne() = assertEquals("1.00e+00", fmt.format(state(1.0, DisplayMode.Sci(2))))

    // ---- ENG mode ----

    @Test fun eng2_thousands() = assertEquals("12.35e+03", fmt.format(state(12345.0, DisplayMode.Eng(2))))
    @Test fun eng2_millions() = assertEquals("1.23e+06", fmt.format(state(1234567.0, DisplayMode.Eng(2))))
    @Test fun eng2_units() = assertEquals("1.23e+00", fmt.format(state(1.23, DisplayMode.Eng(2))))
    @Test fun eng2_small() = assertEquals("1.23e-03", fmt.format(state(0.00123, DisplayMode.Eng(2))))
